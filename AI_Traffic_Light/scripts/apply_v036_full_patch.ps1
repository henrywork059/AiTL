...
- Same-candidate V036 ESP transport repair: replace the raw blocking socket `send()` loop with deadline-bounded `select()` plus `MSG_DONTWAIT`, matching Espressif's non-blocking NetworkClient send strategy so a congested TCP write cannot freeze the camera/control loop; tighten the per-frame freshness deadline from 250 ms to 120 ms.
