...
- Same-candidate V036 R6 transport repair: replace the R5 per-chunk send pattern with one scatter/gather `sendmsg(..., MSG_DONTWAIT)` logical frame write so the 16-byte header is packetized with JPEG data; give each newly accepted TCP connection three bounded warm-up frames (1000 ms no-progress / 1500 ms total) before using 500 ms no-progress / 900 ms total steady-state limits, preventing a 250 ms abort/reconnect loop from repeatedly restarting TCP while retaining a finite stale-frame bound.
