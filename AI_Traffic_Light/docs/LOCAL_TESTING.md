# Local Testing Notes (V031)

V031 / `0_3_1` is the current unaccepted candidate. V030 / `0_3_0` is the previous candidate and V024 / `0_2_4` remains the owner-confirmed passed baseline. Automated checks never promote a candidate.
...
## V031 network policy-arbitration / request-lifecycle repair

Run the two new focused checks in addition to the retained V027-V031 network regressions:

```powershell
& $py ".\scripts\test_network_policy_arbiter.py"
& $py ".\scripts\test_network_service_request_lifecycle.py"
& $py ".\scripts\test_decision_context_request_semantics.py"
```

Acceptance must verify:

- one overlay owner is selected per intersection/tick with priority `incident_hold > pedestrian_crossing > emergency_priority > pedestrian_max_wait > vehicle_class_priority > network_cooperation`;
- ranked scenarios are evaluated once per tick and post-advisory reads use snapshot semantics rather than reapplying scenarios;
- class priority wins over ordinary cooperation when both are triggered in the class-aware mode;
- pedestrian max-wait wins over class/cooperation, while an in-lookahead simulated emergency wins over max-wait unless an active crossing owns clearance protection;
- protected-service request metadata identifies service/source/priority/start and clears when the requested protected service begins;
- lower-priority requests cannot replace a higher-priority pending request;
- evidence records for cooperation/pedestrian/class/emergency carry `context.arbitration` when generated by repaired V031 runs;
- `network_metrics.policy_arbitration` and transition-oriented `policy_arbitration_events` are present;
- the seven modes remain ablation/comparison modes; do not interpret the result as simultaneous class + emergency integration.

### Safer candidate handoff

Prefer applying the changed-files overlay locally, running the complete test runner, and pushing one commit/PR. If GitHub web upload is used, upload every manifest member into the same candidate commit and verify the resulting GitHub files before pulling locally. After explicit acceptance, update `passed_baseline` and optionally tag the accepted commit (for example `passed-0_3_1`) before starting normal next-version work.
