...
The R4 benchmark compares practical ESP32-CAM transport alternatives under the same camera settings so the next production fix is chosen from evidence rather than guesswork.
...
The R4 ATL1 phases deliberately use a minimal raw Python receiver: no UI, inference, image decode, browser relay or PC Studio frame service.
