...
- live `decision_context.requested_service` is populated only from an explicit active `signal_policy.service_request` lifecycle object. A legacy `signal_policy.pending_request` flag by itself is treated as descriptive/stale-capable state and is not promoted to a causal service decision.
...
```json
{
  ...
  "context": {
    ...
    "emergency": {},
    "arbitration": {
      "owner": "network_cooperation",
      "priority": 500,
      "conflict": false,
      "candidates": [{"owner": "network_cooperation", "priority": 500}]
    }
  },
  "provenance": "synthetic_predicted_arrivals",
...
Repaired V031 uses explicit overlay arbitration rather than only a pedestrian-specific lock. Priority is:

```text
incident_hold > pedestrian_crossing > emergency_priority > pedestrian_max_wait > vehicle_class_priority > network_cooperation
```

Only the winning higher-level overlay may mutate timing for that intersection/tick. Lower-priority triggered layers emit defer/protection evidence with zero timing delta. Active crossing protection therefore outranks simulated emergency priority; simulated emergency priority outranks pedestrian max-wait; pedestrian max-wait outranks ordinary class/cooperation. Ranked scenarios remain the lower-level controller base policy and are evaluated once per tick.

Mode results include transition-oriented `policy_arbitration_events`, `network_metrics.policy_arbitration`, and timeline `policy_arbitration` snapshots. Normalized cooperation/pedestrian/class/emergency records include `context.arbitration` when generated by the repaired runtime. The arbitration context is explanatory simulator evidence, not a second physical/live controller.

Repaired benchmark signal snapshots also expose a protected-service lifecycle object:
...
```json
{
  ...
  "service_request": {
    "active": true,
    "request_id": "svc_pedestrian_1_31000",
    "service": "pedestrian",
...
Lower-priority opposite requests are suppressed and the lifecycle clears when the requested protected service begins.
