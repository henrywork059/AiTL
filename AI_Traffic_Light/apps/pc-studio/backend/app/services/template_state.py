...
PC_STUDIO_PAGES: list[dict[str, Any]] = [
    ...
    {"id": "camera_diagnostics", "label": "Camera Diagnostics", "status": "test-ready"},
    ...
    {"id": "simulation_lab", "label": "Simulation Lab", "status": "test-ready"},
...
def get_template_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "implementation_note": (
            "All main PC Studio pages expose working prototype behavior: live/camera tools, one-click camera transport diagnostics, "
            "camera-aligned persistent zones, Live AI zone/signal overlays, zone-aware simulation decisions, occupancy history plus "
            "cross-frame tracking/flow analytics, capture deletion, labeling/training/model management, runtime settings, and real recent logs. "
            "Camera diagnostics isolate ESP control reachability, direct ATL1/JPEG transport, status-poll contention, and the normal managed PC stream path. "
            "Model export, automatic labeling, and physical public-road control remain disabled."
        ),
    }
